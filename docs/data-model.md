> **Source:** [System Design](system-design.md), [PRD](prd.md), and [root idea.md](.../idea.md).
> This document owns the current data contract; annotated legacy notes are historical only.
...
> Traces back to: system design (`system-design.md`), PRD (`prd.md`), and root `idea.md`.
...
Static frontend segments supply map geometry. Supabase `reports` references a segment ID without
cross-database referential integrity. Firestore holds only `users/{uid}.role` for identity and
administrator authorization. The current contract is the report table and RLS section below;
older Firestore diagrams in this document are historical build notes, not current architecture.

## Historical segment notes
...
| `report.photoPath` | User-generated image (potential PII — bystander faces; EXIF stripped) | MVP: none defined `[unverified]` | See Threat T7, `docs/security-compliance.md`. EXIF GPS/camera metadata stripped client-side (BR-008); bystander-face privacy in the photo content itself is **not** mitigated — known gap. |
