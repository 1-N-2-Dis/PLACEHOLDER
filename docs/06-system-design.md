...
> **Build context:** 2-day SparkFest hackathon MVP. Web app + Firebase + MapLibre/OpenFreeMap + ORS + Gemini.
> Scope is deliberately narrow; F-004 (Gemini summary) is P1 stretch.

## Context diagram

The system is a single-page web app served from Firebase Hosting. Map rendering and routing are
non-Google (MapLibre GL JS + OpenFreeMap vector tiles for render, OpenRouteService for foot
routing). It talks to Google surfaces for the rest: Cloud Firestore (segment reports) and the
Gemini API (P1 risk summary), plus Firebase Auth/Hosting. All actors are commuters in the single
PUP Sta. Mesa zone (BR-003).
...
|-----------|----------------|------|------------|
| **Web app (React + Vite)** | Renders zone map, segment flags, report form, route-check result | UI state, client validation (condition-only fields, BR-001) | Firebase JS SDK, MapLibre GL JS, ORS |
...
1. App loads → MapLibre renders the Sta. Mesa zone → app reads current segment flags from Firestore.
...
| **MapLibre GL + OpenFreeMap** | Keyless, free vector tiles + open-source renderer; no billing setup for F-001/F-003 | Fewer built-in POIs / places data vs Google; some map features are DIY | Google Maps Platform (needs billing + restricted key) |
| **OpenRouteService (ORS)** | Free foot-walking routing with avoid-features to steer around flagged/highway-class legs | API key ships in client bundle; usage rate limits (capped at 2 calls/request) | Google Maps Platform (needs billing + restricted key) |
...
- **MapLibre GL + OpenFreeMap** — MapLibre GL JS loads OpenFreeMap vector tiles in-browser; keyless, no billing. Failure mode: tile endpoint down → fall back to cached/static tiles and a clear error state.
- **OpenRouteService (ORS)** — foot-walking directions called from the client; API key ships in the bundle (not yet referrer-restricted — open item). Failure mode: quota/latency → show flagged segments unrouted (skip the drawn route line).
...
3. **OpenRouteService (ORS) API key** — exposed in client bundle by design; **must be restricted by HTTP referrer/origin + usage-capped**, else the key is abusable/billable by anyone. Currently **NOT yet referrer-restricted → open security item.** This is the only viable authz for a browser-shipped key.
...
### Proposed ADRs
- ADR: MapLibre/OpenFreeMap + ORS over Google Maps Platform (keyless, cost).
...
2. Add MapLibre GL + OpenFreeMap (keyless) render for the Sta. Mesa zone; wire ORS routing and **restrict/cap the ORS key** (referrer/origin + usage cap) as soon as it's live (F-001).
...
> Guardrail: keep auth/rules and key restriction in steps 2 and 4 — they are gates, not polish. A world-writable Firestore or an unrestricted ORS key is a demo-day liability.
