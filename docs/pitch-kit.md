...
| **Key partners** | PUP GAD office + student orgs; LGU GAD/women's desks; women's organizations. Infra: MapLibre/OpenFreeMap, a client-side Rust/WASM routing engine, Firebase, Gemini. | Model |
...
| **3:15–4:15** | **Model + payer** | Free to women — they shouldn't pay for safety. Institutional funding through PUP GAD, LGU GAD desks, or grants is a hypothesis to test; no payer has committed and operating economics remain unvalidated. | Sustainability |
...
| "What's your tech stack — and where's the Google technology?" | Jim | React+Vite front; **MapLibre GL + OpenFreeMap** for maps (keyless, non-Google, by choice) and a **client-side Rust/WebAssembly routing engine** we built ourselves — no external routing API at all, so it works even if a network provider is down; **Firebase (Auth + Firestore) + Gemini** satisfy the Google-tech requirement; Vercel + Render host it. |
...
| "How is this different from Google Maps / Safetipin?" | Alex | Preventive, first/last-mile, hyperlocal, conditions-only. Safetipin is a useful comparison, but its funding model does not validate GuidHer's payer model; GuidHer still needs a committed payer. |
