> **Source:** [root idea.md](.../idea.md) and [context.md](../context.md). This document owns
> current product behavior; historical build notes are not a substitute for the active contract.
...
- **Firebase** (Auth + Firestore role records) — identity and authorization. Together with
  Gemini, it satisfies the Google-technology requirement.
- **Supabase/Postgres** — report, analytics, and reference-table data. Browser reads are RLS-gated;
...
- **Maps/routing are non-Google:** map render + segment overlays = **MapLibre GL + OpenFreeMap** (keyless); route check/recommendation = a **client-side Rust/WASM routing engine** (F-001, F-003, F-005), replacing OpenRouteService (ADR-0003). The map/routing stack is owned by [System Design](./system-design.md).
...
| F-002 | One-tap report of a route/segment condition (poor lighting / no crowd / recent incident) | P0 | "Lived danger is never written down" (idea §4) | Firebase-authenticated user → `submitReport` → server-only Supabase write. Condition flags only; no crime labels. The form requires title + note; optional photo remains disabled. |
...
| F-008 | AI-assessed "is my route safe tonight?" for the recommended route | P0 | "The pre-trip check was disconnected from the actual route being taken, and a raw flag list isn't the same as an answer" | `RouteCheck.jsx` explicitly calls `assessRoute`; the Express API reads real on-route Supabase reports and returns a Gemini verdict constrained not to invent facts or promise rescue. |
| F-009 | Account upgrade and remove-only administrator moderation | P1 | "A harmful or invalid report needs accountable removal" | Firebase Auth retains identity; Firestore `users/{uid}.role` supports server-side authorization; reports remain Supabase-owned. |
| F-010 | Community heatmap layer of validated (yellow/red severity) reports | P1 | "A raw list of pins doesn't show where risk clusters at a glance" | Client-side reduction over fresh, Supabase-sourced reports joined to static segment geometry. It shows transient conditions only; it does not store or label a place-level rating. |
| F-011 | Confirmed 911 quick-dial handoff | P1 | "A commuter may need a direct way to use the device dialer" | Client-only `tel:911` handoff with misclick confirmation; no dispatch, tracking, or rescue promise. |
...
- **BR-005** — Report submission requires an authenticated user (Firebase Auth) to enable
  ...
  **Amended (F-006):** writes now happen exclusively via the `submitReport` route (`backend/server`)
  into Supabase, never from a browser client. Supabase RLS denies direct client writes. A verified Firebase ID token is still required
...
- **BR-008** — Photo evidence (F-007) must have EXIF metadata (GPS, camera info) stripped
  ...
  stripping (see `docs/security-compliance.md` Threat T7).
...
- **INV-001:** conditions only; no stored or rendered crime-zone/place rating.
- **INV-002:** no in-app rescue, dispatch, tracking, or intervention promise; F-011 is only a
  confirmed handoff to the device dialer.
- **INV-003:** active coverage remains the PUP Sta. Mesa commute zone.
- **INV-004:** Gemini output remains grounded in supplied report or route inputs.
- **INV-005:** severity remains a per-report triage attribute, never a place score.
...
3. User taps "Is my route okay tonight?" (bottom-center map overlay, `RouteCheck.jsx`) and then
   ...
   ID only) to `assessRoute`; the API reads each one's real Supabase report and returns a
...
- **F-002:** An authenticated user can flag a selected segment with one of {poor lighting, no
  crowd, recent incident}, a required title (≤60 chars), and a required note; the server persists
  the report to Supabase and it appears on the map only after F-006 validation; no free-form crime-label field
  exists (BR-001).
...
- **F-008:** `assessRoute` reads each on-route segment's real, current Supabase report (never
...
- **F-010:** Given at least one report with `severity` yellow or red within the freshness window,
  ...
  stores, police presence), toggled independently — see `docs/data-model.md`.
...
- **Firebase** — Auth and Firestore role records. Project setup `[unverified]`.
- **Supabase** — report/analytics tables with browser read access and server-only writes.
