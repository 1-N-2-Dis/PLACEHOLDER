> **Historical SparkFest artifact.** This sprint board is not the current task authority. Use
> [implementation-plan.md](implementation-plan.md), [ops.md](ops.md), and
> [system-design.md](system-design.md) for current work.
...
> [START-HERE](./onboarding.md) → this board → [POSTMORTEM](./DECISION-LEDGER.md) (history/why) →
> [pitch kit](./pitch-kit.md). The rubric we're scored on:
...
- [ ] **Run the interview probe sheet with real PUP women** ([pitch kit §5](./pitch-kit.md)). This is the riskiest assumption *and* the judges' first question ("is the problem validated?"). Bring ≥1 real quote to the pitch.
- [ ] Rehearse the 5-min script + confirm every Q&A owner knows their rows ([pitch kit §3–§4](./pitch-kit.md)).
- [ ] Housekeeping: resolve the git remote (currently a placeholder) and push the doc work.
- Done → BMC, narrative, pitch script, Q&A map, probe sheet, playbook (see [pitch kit](./pitch-kit.md)).
...
- [x] Verify the **core loop end-to-end**: open map → conditions → submit report → route adjusts, with **2 routes max**, recommended (safest computed) path **+ visible alternative**, and "recommended = safest we found" copy (F-005). Done via the client-side WASM routing engine (ADR-0003) — verified in-browser 2026-07-06, see [POSTMORTEM §3](./DECISION-LEDGER.md).
- [x] Confirm the **PRD F-005 line reads "2 routes"** (was 2–3); if you change it, add a line to [POSTMORTEM §3](./DECISION-LEDGER.md). Done as part of ADR-0003.
...
- [ ] Own and rehearse the AI Q&A rows: "how does the AI work," "isn't this a crime map," "is the data real" ([pitch kit §4](./pitch-kit.md)).
...
- [ ] **Reach out to Ate Ayen and Kuya Troy** for UI/UX and branding improvements; capture their input and fold actionable changes into the demo screens (and record who suggested what in [POSTMORTEM §3](./DECISION-LEDGER.md)).
...
- [x] Name + stack consistent across key docs; decision log seeded → [POSTMORTEM](./DECISION-LEDGER.md).
- [x] BMC + pitch script + Q&A map + probe sheet ready → [pitch kit](./pitch-kit.md).
...
- **What changed and why** → [POSTMORTEM.md](./DECISION-LEDGER.md) (pivots, naming, TRUE/UNVALIDATED table).
