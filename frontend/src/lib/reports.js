...
//
// PROTOTYPE: Firebase is stubbed (firebase.js), so this module delegates to mockReports.js
// which uses localStorage. Swap back to the Firestore implementation (commented below) once
// a real Firebase project is wired in via .env.
//
...
export { subscribeReports, latestBySegment } from './mockReports.js';

// F-009 admin moderation: remove a report (stub — no-op in prototype).
export function deleteReport(/* id */) {
  return Promise.resolve();
}

// ── Firestore implementation (restore when .env Firebase config is present) ──────────────────
