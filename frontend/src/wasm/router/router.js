 ...
 * @returns {string}
 */
export function find_routes(start_lng, start_lat, end_lng, end_lat) {
    let deferred2_0;
    let deferred2_1;
    try {
        const ret = wasm.find_routes(start_lng, start_lat, end_lng, end_lat);
        var ptr1 = ret[0];
        var len1 = ret[1];
        if (ret[3]) {
            ptr1 = 0; len1 = 0;
 ...
 * @param {string} json
 */
export function load_graph_geojson(json) {
    const ptr0 = passStringToWasm0(json, wasm.__wbindgen_malloc, wasm.__wbindgen_realloc);
    const len0 = WASM_VECTOR_LEN;
    const ret = wasm.load_graph_geojson(ptr0, len0);
    if (ret[1]) {
        throw takeFromExternrefTable0(ret[0]);
    }
}
...
async function __wbg_init(module_or_path) {
    ...
    if (module_or_path === undefined) {
        module_or_path = new URL('router_bg.wasm', import.meta.url);
    }
    const imports = __wbg_get_imports();
